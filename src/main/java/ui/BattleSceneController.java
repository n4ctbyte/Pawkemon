...
public class BattleSceneController implements BattleLogger.LogListener {
    ...
    @FXML private Pane darknessOverlay;
    ...
    @FXML private Button surrenderButtonP1;
    @FXML private Button surrenderButtonP2;
    ...
    public void initialize() {
        ...
        darknessOverlay.setOpacity(0.0);
        darknessOverlay.setMouseTransparent(true);
        ...
        surrenderButtonP1.setVisible(true);
        surrenderButtonP2.setVisible(true);
    ...
    private void executeTurn(Hero user, Hero target) {
        ...
        Runnable onAnimationFinished = () -> {
            ...
            if ((targetType == TargetType.ALL_ALLIES || targetType == TargetType.ALL_ENEMIES)) {
                ...
                try {
                    ...
                    try { selectedSkill.use(user, targets.get(0)); } catch (Exception ex) {}
                } catch (Exception e) {}
            }

            boolean isHarmful = (targetType == TargetType.SINGLE_ENEMY || targetType == TargetType.ALL_ENEMIES);
        ...
        if (isHarmful) {
            List<HeroCardController> targetCards = targets.stream()
                                                        .map(this::getCardForHero)
                                                        .collect(Collectors.toList());
            playJoltAndSpotlightAnimation(userCard, targetCards, onAnimationFinished);
        } else {
    ...
    private void playJoltAndSpotlightAnimation(HeroCardController userCard, List<HeroCardController> targetCards, Runnable onFinishedCallback) {
        
        List<VBox> allCards = Stream.concat(player1Cards.stream(), player2Cards.stream())
                                  .map(HeroCardController::getRoot)
                                  .collect(Collectors.toList());
        
        List<VBox> targetVBoxes = targetCards.stream()
                                          .map(HeroCardController::getRoot)
                                          .collect(Collectors.toList());
        
        VBox attackerVBox = userCard.getRoot();
        
        List<VBox> otherCards = allCards.stream()
                                      .filter(box -> box != attackerVBox && !targetVBoxes.contains(box))
                                      .collect(Collectors.toList());

        ParallelTransition dimBackgroundAndCardsAndJolt = new ParallelTransition();
        dimBackgroundAndCardsAndJolt.getChildren().add(createBackgroundFadeAnimation(0.7, Duration.millis(300))); 
        for (VBox card : otherCards) {
            FadeTransition ft = new FadeTransition(Duration.millis(300), card);
            ft.setToValue(0.4);
            dimBackgroundAndCardsAndJolt.getChildren().add(ft);
        }
        dimBackgroundAndCardsAndJolt.getChildren().add(createJoltAnimation(userCard));
        
        FadeTransition attackerFadeIn = new FadeTransition(Duration.millis(1), attackerVBox);
        attackerFadeIn.setToValue(1.0);
        dimBackgroundAndCardsAndJolt.getChildren().add(attackerFadeIn);

        for (VBox targetVBox : targetVBoxes) {
            FadeTransition targetFadeIn = new FadeTransition(Duration.millis(1), targetVBox);
            targetFadeIn.setToValue(1.0);
            dimBackgroundAndCardsAndJolt.getChildren().add(targetFadeIn);
        }

        ParallelTransition shakeTargets = new ParallelTransition();
        for(VBox targetVBox : targetVBoxes) {
            shakeTargets.getChildren().add(createShakeAnimation(targetVBox));
        }
        
        ParallelTransition undimBackgroundAndCards = new ParallelTransition();
        undimBackgroundAndCards.getChildren().add(createBackgroundFadeAnimation(0.0, Duration.millis(200))); 
        for (VBox card : otherCards) {
            FadeTransition ft = new FadeTransition(Duration.millis(200), card);
            ft.setToValue(1.0);
            undimBackgroundAndCards.getChildren().add(ft);
        }
        
        SequentialTransition fullAnimation = new SequentialTransition(
            dimBackgroundAndCardsAndJolt,
            new PauseTransition(Duration.millis(100)),
            shakeTargets,
            new PauseTransition(Duration.millis(300)),
            undimBackgroundAndCards
        );
    ...
    private SequentialTransition createJoltAnimation(HeroCardController userCard) {
        VBox attackerVBox = userCard.getRoot();
        boolean isP1Attacking = player1Cards.contains(userCard);
        
        TranslateTransition joltForward = new TranslateTransition(Duration.millis(150), attackerVBox);
        joltForward.setByX(isP1Attacking ? 30 : -30);
        joltForward.setInterpolator(Interpolator.EASE_OUT);

        TranslateTransition joltBack = new TranslateTransition(Duration.millis(100), attackerVBox);
        joltBack.setToX(0);
        
        userCard.showAnimation(selectedSkill.getName());
        
        return new SequentialTransition(joltForward, joltBack);
    }
    
    private TranslateTransition createShakeAnimation(VBox targetVBox) {
        TranslateTransition shake = new TranslateTransition(Duration.millis(50), targetVBox);
        shake.setByX(10);
        shake.setCycleCount(6);
        shake.setAutoReverse(true);
        shake.setInterpolator(Interpolator.EASE_BOTH);
        shake.setOnFinished(e -> targetVBox.setTranslateX(0));
        return shake;
    }
    
    private FadeTransition createBackgroundFadeAnimation(double targetOpacity, Duration duration) {
        FadeTransition ft = new FadeTransition(duration, darknessOverlay);
    ...
    private void onSkillButtonClicked(Skill skill) {
        ...
        if (targetType == TargetType.SELF) {
            executeTurn(selectedHero, selectedHero);
    ...
    private void onHeroCardClicked(HeroCardController clickedCard) {
        ...
        switch (currentState) {
            ...
            case SELECTING_TARGET:
                ...
                if (isValidTarget) {
                    executeTurn(selectedHero, clickedHero);
    ...
    private void endGame(Player winner) {
        ...
        backButton.setVisible(false);
        surrenderButtonP1.setVisible(false);
        surrenderButtonP2.setVisible(false);
    ...
    @FXML
    private void onSurrenderP1Clicked() {
        MusicManager.getInstance().playClickSound();
        log("Player 1 telah menyerah!");
        endGame(player2);
    }

    @FXML
    private void onSurrenderP2Clicked() {
        MusicManager.getInstance().playClickSound();
        log("Player 2 telah menyerah!");
        endGame(player1);
    }
