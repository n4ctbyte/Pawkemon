...
public class SplashScreenController {
    ...
    private void playTypingAnimation() {
        ...
        pt1_startDelay.setOnFinished(event -> {
            if (typingAudioPlayer != null) {
        ...
        pt2_postTypingDelay.setOnFinished(event -> {
            MusicManager.getInstance().playLogoSound();
        });
    ...
    private void loadNextScene() {
        ...
        try {
            Parent splash2Root = FXMLLoader.load(getClass().getResource("SplashScreen2.fxml")); 
            rootPane.getScene().setRoot(splash2Root);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
